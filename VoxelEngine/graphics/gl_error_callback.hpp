...
namespace ve {
    static void GLAPIENTRY gl_error_callback(
        ...
        if (contains(type, GL_DEBUG_TYPE_ERROR, GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR)) level = logger::level::ERROR;
        if (contains(type, GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR, GL_DEBUG_TYPE_PERFORMANCE, GL_DEBUG_TYPE_PORTABILITY)) level = logger::level::WARNING;
